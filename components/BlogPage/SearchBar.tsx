  ...
  return (
    <div className="grow xl:max-w-96">
      <div className="static top-32 flex flex-col gap-y-4 xl:sticky">
        ...
        {/* filter options */}
        <ul className="flex max-w-full flex-row items-start justify-start overflow-y-scroll xl:flex-col xl:items-stretch xl:justify-center">
          {searchOptions.map((option, index) => (
            <li
              className={`flex h-[3rem] cursor-pointer items-center rounded-full px-4 text-xl transition-all duration-200 xl:rounded-md ${selected == option && "bg-gray-200"}`}
